...
def nat_gcd():
    def sum(a, b):
        a1 = Natural(str(a))
        b1 = Natural(str(b))
        text = "GCD(",a1,",",b1,")=", Natural.gcd(a1,b1)
        return text
...
def nat_lcm():
    def sum(a, b):
        a1 = Natural(str(a))
        b1 = Natural(str(b))
        text = "LCM(",a1,",",b1,")=", Natural.lcm(a1, b1)
        return text
...
def int_abs():
    ...
    def division(a):
        a1 = Integer(str(a))
        text = ' модуль числа= ',Integer.abs(a1)
        return text
...
def pol_divis():
    ...
    win = Toplevel(root)
    win.title("Деление двух многочленов (ввод коэффициентов)")
    win.resizable(width=False, height=False)
    frame = Frame(win)
    frame.grid()
...
def pol_gcd():
    def sum(a, b):
        a1 = Polynome(str(a))
        b1 = Polynome(str(b))
        text = "GCD(",a1,",",b1,")=", Polynome.gcd(a1,b1)
        return text
...
def pol_der():
    def division(a):
        a1 = Polynome(str(a))
        text = "(",a1,")'=", Polynome.derivative(a1)
        return text
...
fm = Menu(m) #меню для натуральных чисел
...
hm = Menu(m)  # второй пункт меню , целые числа
...
dm = Menu(m) #и еще один пункт, ого (Дроби)
